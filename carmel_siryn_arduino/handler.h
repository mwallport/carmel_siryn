...
class handler
{
  ...
  int sndCmd(HardwareSerial& so, uint8_t*, uint8_t);
  
  // handle reading a response to a write command
  // read into the m_rx_buff and return bytes read
  int rcvWriteResp(HardwareSerial& so, size_t min_pkt_size, uint8_t* rx_buff, uint8_t rx_buff_size);
  
  // handle reading a response to a read command - handle the ByteCount in the Rx'ed packet
  // read into the m_rx_buff and return bytes read
  int rcvReadResp(HardwareSerial& so, size_t min_pkt_size, uint8_t* rx_buff, uint8_t rx_buff_size);
};
