/***************************************************
  ...
  Modified for deftDevise to enable user to effectivlye use DRDY
  - added member functions suffixed with _dd ( for (d)eft(d)evise
...
/*!
   ...
   technique
    ...
    430 or 4300
    @returns Temperature in C
    @added for deftDevise which is leveraging the DRDY pin
*/
/**************************************************************************/
float Adafruit_MAX31865::temperature_dd(float RTDnominal, float refResistor) {
  float Z1, Z2, Z3, Z4, Rt, temp;
...
uint16_t Adafruit_MAX31865::readRTD(void) {
  clearFault();
  enableBias(true); // TODO: put this back
  delay(10);        // TODO: put this back
  uint8_t t = readRegister8(MAX31865_CONFIG_REG);
  t |= MAX31865_CONFIG_1SHOT;
  writeRegister8(MAX31865_CONFIG_REG, t);
  delay(65);        // TODO: put this back

  uint16_t rtd = readRegister16(MAX31865_RTDMSB_REG);

  enableBias(false); // Disable bias current again to reduce selfheating. TODO: put this back
...
/**************************************************************************/
/*!
    @initiateConversion_dd, the following is assumed to be done by caller
      - Vbias enabled is configured for the RTD
      - using program has DRDY pin attached and monitoring it to know when a
        conversion has completed
      - trying to eliminate un-needed calls to improve performance
          - not call calling clearFalut
          - not calling enableBias as it is assumed to be already enabled
          - no 10ms delay as the Vbias is already enabled
          - no 65ms delay after writing register 16, using the DRDY ISR

    @return The raw unsigned 16-bit value, NOT temperature!
*/
/**************************************************************************/
uint16_t Adafruit_MAX31865::readRTD_dd(void) {

  // read register 16 to start a convesion
  uint16_t rtd = readRegister16(MAX31865_RTDMSB_REG);
...
/**********************************************/
/*!
    @ member function to clear the one-shot
*/
/**********************************************/
void Adafruit_MAX31865::clearOneShot_dd(void) {
...
*/
/**********************************************/
void Adafruit_MAX31865::setOneShot_dd(void) {
